export const en = {
  // Common
  common: {
    cancel: 'Cancel',
    save: 'Save',
    edit: 'Edit',
    delete: 'Delete',
    add: 'Add',
    search: 'Search',
    loading: 'Loading',
    back: 'Back',
    viewAll: 'View All',
    confirm: 'Confirm',
    yes: 'Yes',
    no: 'No',
  },

  // Navigation
  nav: {
    home: 'Home',
    history: 'History',
    add: 'Add',
    askAI: 'Fin Buddy',
    profile: 'Profile',
  },

  // Auth
  auth: {
    welcomeToSpendo: 'Welcome to BeeWise',
    signInDescription: 'Sign in to your account to manage your finances wisely',
    email: 'Email',
    emailPlaceholder: 'Enter your email',
    password: 'Password',
    passwordPlaceholder: 'Enter your password',
    signIn: 'Sign In',
    signingIn: 'Signing in...',
    signUp: 'Sign up',
    signOut: 'Sign Out',
    signingOut: 'Signing out...',
    dontHaveAccount: "Don't have an account?",
    alreadyHaveAccount: 'Already have an account?',
    createAccount: 'Create Account',
    createAccountDescription: 'Sign up for BeeWise to start your financial literacy journey',
    creatingAccount: 'Creating account...',
    invalidCredentials: 'Invalid email or password',
    emailInUse: 'Email is already registered',
    weakPassword: 'Password is too weak',
    passwordMinLength: 'Password must be at least 6 characters',
    passwordsDontMatch: 'Passwords do not match',
    failedToCreate: 'Failed to create account',
    fullName: 'Full Name',
    fullNamePlaceholder: 'Enter your full name',
    confirmPassword: 'Confirm Password',
    confirmPasswordPlaceholder: 'Confirm your password',
    createPasswordPlaceholder: 'Create a password',
  },

  // Dashboard
  dashboard: {
    spendo: 'BeeWise',
    welcomeBack: 'Welcome back',
    balance: 'Balance',
    income: 'Income',
    expenses: 'Expenses',
    thisMonth: 'This month',
    recentTransactions: 'Recent Transactions',
    noTransactions: 'No transactions yet',
    addFirstTransaction: 'Add Your First Transaction',
    loadingDashboard: 'Loading dashboard...',
    today: 'Today',
    yesterday: 'Yesterday',
    daysAgo: 'days ago',
  },

  // Transactions
  transactions: {
    transactions: 'Transactions',
    viewManage: 'View and manage your transactions',
    searchPlaceholder: 'Search transactions...',
    noTransactionsFound: 'No transactions found',
    loadingTransactions: 'Loading transactions...',
    addTransaction: 'Add Transaction',
    editTransaction: 'Edit Transaction',
    deleteConfirm: 'Are you sure you want to delete this transaction?',
    deleteFailed: 'Failed to delete transaction',
    updateFailed: 'Failed to update transaction',
    recordNew: 'Record a new income or expense',
  },

  // Categories
  categories: {
    categories: 'Categories',
    manageCategories: 'Manage your expense categories',
    expenses: 'Expenses',
    income: 'Income',
    loadingCategories: 'Loading categories...',
    addCategory: 'Add Category',
    editCategory: 'Edit Category',
    deleteConfirm: 'Are you sure you want to delete this category?',
    noExpenseCategories: 'No expense categories yet',
    noIncomeCategories: 'No income categories yet',
    addFirstCategory: 'Add Your First Category',
  },

  // Reports
  reports: {
    reports: 'Reports',
    period: 'Period',
    totalIncome: 'Total Income',
    totalExpenses: 'Total Expenses',
    netBalance: 'Net Balance',
    expenseCategories: 'Expense Categories',
    incomeCategories: 'Income Categories',
    transactions: 'transactions',
    ofTotalExpenses: 'of total expenses',
    ofTotalIncome: 'of total income',
    noTransactionsFound: 'No transactions found for',
    loadingReports: 'Loading reports...',
  },

  // Profile
  profile: {
    profile: 'Profile',
    manageAccount: 'Manage your account and preferences',
    memberSince: 'Member since',
    recently: 'Recently',
    quickOverview: 'Quick Overview',
    transactionsCount: 'Transactions',
    categoriesCount: 'Categories',
    manageCategories: 'Manage Categories',
    manageCategoriesDesc: 'Add, edit, or delete expense categories',
    reportsAnalytics: 'Reports & Analytics',
    reportsAnalyticsDesc: 'View spending patterns and insights',
    appSettings: 'App Settings',
    appSettingsDesc: 'Preferences and configuration',
    privacySecurity: 'Privacy & Security',
    privacySecurityDesc: 'Account security settings',
    helpSupport: 'Help & Support',
    helpSupportDesc: 'Get help and contact support',
    appVersion: 'BeeWise v1.0.0',
    builtWith: 'Powered by SLIIT Kandy Campus',
    language: 'Language',
    languageDesc: 'Choose your preferred language',
    currency: 'Currency',
    currencyDesc: 'Choose your preferred currency',
  },

  // Forms
  forms: {
    type: 'Type',
    amount: 'Amount',
    amountPlaceholder: '0.00',
    category: 'Category',
    noCategories: 'No {type} categories available. Create one first.',
    description: 'Description (Optional)',
    descriptionPlaceholder: 'Enter description',
    date: 'Date',
    saving: 'Saving...',
    categoryName: 'Category Name',
    categoryNamePlaceholder: 'Enter category name',
    icon: 'Icon',
    selectIcon: 'Select an icon',
    enterValidAmount: 'Please enter a valid amount',
    selectCategory: 'Please select a category',
  },

  // Months
  months: {
    january: 'January',
    february: 'February',
    march: 'March',
    april: 'April',
    may: 'May',
    june: 'June',
    july: 'July',
    august: 'August',
    september: 'September',
    october: 'October',
    november: 'November',
    december: 'December',
  },

  // Game
  game: {
    title: 'Student Loan Management Game',
    subtitle: 'Make smart financial decisions',
    groupNumber: 'Group Number',
    enterGroup: 'Enter your group number (1-10)',
    startGame: 'Start Game',
    changeGroup: 'Change Group',
    reset: 'Reset',
    resetConfirm: 'Reset game?',
    step: 'Step',
    of: 'of',
    selectChoice: 'Select Your Choice:',
    previousChoices: 'Your Previous Choices',
    confirmChoice: 'Confirm Choice',
    confirmMessage: 'Are you sure you want to select this option? This choice cannot be changed!',
    confirmAndLock: 'Confirm & Lock',
    warningDeadEnd: 'This choice may lead to a dead end!',
    gameComplete: 'Game Complete!',
    deadEnd: 'Dead End!',
    deadEndMessage: 'This choice led to financial problems',
    results: 'Results',
    financialSummary: 'Financial Summary',
    totalLoan: 'Total Loan',
    monthlyEMI: 'Monthly EMI',
    remainingDebt: 'Remaining Debt',
    savings: 'Savings',
    yearsToComplete: 'Years to Complete',
    years: 'years',
    recommendations: 'Recommendations',
    keyLearnings: 'Key Learnings',
    scenarioProfile: 'Scenario Profile',
    excellent: 'EXCELLENT',
    good: 'GOOD',
    fair: 'FAIR',
    poor: 'POOR',
    failed: 'FAILED',
  },

  // Ask AI
  askAI: {
    title: 'AI Financial Advisor',
    subtitle: 'Get personalized advice based on your spending',
    inputPlaceholder: 'Ask about your spending...',
    sendMessage: 'Send',
    thinking: 'Thinking...',
    suggestedQuestions: 'Suggested Questions',
    quickInsights: 'Quick Financial Snapshot',
    noTransactionsYet: 'Add some transactions first to get personalized financial advice',
    addTransactionsButton: 'Add Transactions',
    errorLoadingData: 'Failed to load your financial data',
    errorSendingMessage: 'Failed to send message. Please try again.',
    errorNoApiKey: 'AI feature is not configured. Please add your OpenAI API key.',
    setupInstructions: 'Add NEXT_PUBLIC_OPENAI_API_KEY to your .env.local file',
    rateLimitError: 'Too many requests. Please wait a moment.',
    thisMonth: 'This Month',
    topSpending: 'Top Spending',
    analyzeMore: 'Analyze your spending patterns',
    chatHistory: 'Chat History',
    newChat: 'New Chat',
    clearChat: 'Clear Chat',
    emptyState: 'Start by asking a question about your finances or tap a suggested question above',
  },

  // Loan Calculator
  loanCalculator: {
    title: 'Loan Calculator',
    subtitle: 'Calculate your loan EMI and repayment schedule',
    loanAmount: 'Loan Amount',
    loanAmountPlaceholder: 'Enter loan amount',
    interestRate: 'Interest Rate (% per annum)',
    interestRatePlaceholder: 'Enter interest rate',
    loanTenure: 'Loan Tenure',
    months: 'Months',
    years: 'Years',
    monthsShort: 'Months',
    yearsShort: 'Years',
    calculateEMI: 'Calculate EMI',
    calculating: 'Calculating...',
    results: 'Calculation Results',
    monthlyEMI: 'Monthly EMI',
    principalAmount: 'Principal Amount',
    totalInterest: 'Total Interest',
    totalAmount: 'Total Amount Payable',
    loanType: 'Loan Type',
    personalLoan: 'Personal Loan',
    homeLoan: 'Home Loan',
    vehicleLoan: 'Vehicle Loan',
    educationLoan: 'Education Loan',
    businessLoan: 'Business Loan',
    otherLoan: 'Other Loan',
    typicalRates: 'Typical Rates in Sri Lanka',
    personalRate: 'Personal: 12-18% p.a.',
    homeRate: 'Home: 8-12% p.a.',
    vehicleRate: 'Vehicle: 10-15% p.a.',
    educationRate: 'Education: 6-10% p.a.',
    businessRate: 'Business: 14-20% p.a.',
    ratesDisclaimer: 'Interest rates may vary based on your credit score, income level, employment status, and overall risk profile. The rates shown are indicative market averages. Please consult with your financial institution for personalized rates.',
    invalidAmount: 'Please enter a valid loan amount',
    invalidRate: 'Please enter a valid interest rate',
    invalidTenure: 'Please enter a valid tenure',
    breakdown: 'Payment Breakdown',
    principal: 'Principal',
    interest: 'Interest',
    loanCalculatorDesc: 'Calculate EMI and plan your loan',
    quickAmounts: 'Quick Amounts',
    lakh: 'Lakh',
    lakhs: 'Lakhs',
    crore: 'Crore',
    clearAmount: 'Clear',
  },

  // Fraud Checker
  fraudChecker: {
    title: 'Red Noticed Companies',
    subtitle: 'Check if a company is flagged as fraudulent',
    menuDescription: 'Verify suspicious investment companies',
    searchPlaceholder: 'Search company name...',
    infoBanner: 'If you received an investment or business opportunity message, search the company name here to check if it has been flagged.',
    noResults: 'No flagged companies found',
    noResultsHint: 'This company is not in our database. Stay cautious and do your own research before investing.',
    resultsCount: '{count} of {total} companies match',
    totalCount: '{count} companies flagged',
    alsoKnownAs: 'Also known as',
    disclaimer: 'This list is for awareness purposes only. Absence from this list does not guarantee a company is legitimate. Always do thorough research before investing.',
    riskLevels: {
      critical: 'Critical',
      high: 'High Risk',
    },
    scamTypes: {
      pyramid: 'Pyramid',
      ponzi: 'Ponzi',
      investment_fraud: 'Investment Fraud',
      forex_scam: 'Forex Scam',
      crypto_scam: 'Crypto Scam',
      mlm_scam: 'MLM Scam',
    },
    filterAll: 'All',
    tabs: {
      searchCompanies: 'Search Companies',
      analyzeMessage: 'Analyze Message',
    },
    smsAnalyzer: {
      subtitle: 'Paste an investment SMS to instantly check for fraud signals',
      textareaPlaceholder: 'Paste the SMS or investment message here...',
      pasteButton: 'Paste from Clipboard',
      clearButton: 'Clear',
      analyzeButton: 'Analyze Message',
      analyzing: 'Analyzing...',
      resultTitle: 'Analysis Result',
      riskScore: 'Risk Score',
      verdictLikelyFraud: 'Likely Fraud',
      verdictSuspicious: 'Suspicious',
      verdictAppearsSafe: 'Appears Safe',
      detectedRedFlags: 'Detected Red Flags',
      noRedFlags: 'No red flags detected',
      matchedCompaniesWarning: 'Warning: Known fraudulent companies detected in this message!',
      matchedCompaniesTitle: 'Matched Companies',
      severityHigh: 'High',
      severityMedium: 'Medium',
      severityLow: 'Low',
      matchedPhrases: 'Matched phrases',
      aiAnalysisTitle: 'AI Analysis',
      aiPoweredBy: 'Powered by OpenAI',
      aiEnhancing: 'AI is enhancing the analysis...',
      disclaimer: 'This analysis is for awareness purposes only. Results are based on pattern matching and may not be 100% accurate. Always verify investment opportunities with official regulatory bodies (SEC, CBSL).',
      pasteNotSupported: 'Clipboard paste not supported on this device',
      messageRequired: 'Please paste a message to analyze',
      indicatorCategories: {
        guaranteedReturns: 'Guaranteed Returns',
        unrealisticReturns: 'Unrealistic Returns',
        urgencyPressure: 'Urgency / Pressure',
        recruitmentReferral: 'Recruitment / Referral',
        suspiciousContact: 'Suspicious Contact',
        legitimacyFlags: 'Legitimacy Red Flags',
        investmentJargon: 'Investment Jargon Misuse',
      },
    },
  },

  // Launch
  launch: {
    title: 'Wake the Hive',
    subtitle: 'Awakening Financial Wisdom',
    activatePlatform: 'Activate BeeWise',
    connectEcosystem: 'Connecting features...',
    launching: 'BeeWise is Live',
    readyForUsers: 'Ready for 10,000+ users',
    enterApp: 'Enter BeeWise',
    clickToActivate: 'Click to activate',
    clickToContinue: 'Click to continue',
    clickToLaunch: 'Click to launch',
    // Cell labels
    aiPowered: 'AI-Powered Platform',
    expenseTracking: 'Expense Tracking',
    aiAdvisor: 'AI Financial Advisor',
    smartBudgeting: 'Smart Budgeting',
    learningModules: 'Learning Modules',
    investmentInsights: 'Investment Insights',
    reportsAnalytics: 'Reports & Analytics',
    multiCurrency: 'Multi-currency',
    billReminders: 'Bill Reminders',
    goalSetting: 'Goal Setting',
    debtManagement: 'Debt Management',
    taxCalculator: 'Tax Calculator',
    loanAnalysis: 'Loan Analysis',
    savingsOptimizer: 'Savings Optimizer',
    creditMonitoring: 'Credit Monitoring',
    exportTools: 'Export Tools',
    community: 'Community',
    securityFirst: 'Security First',
    crossPlatform: 'Cross-platform',
  },
};

export type TranslationKeys = typeof en;
